...
#'
#' @param pres_bin_stack a raster stack of presence-absence. It can be an object of class "raster" or "SpatRaster".
...
rast_pd <- function(pres_bin_stack, tree){
  {
    ...
    tree_phy4 <- phylo4(tree) # criei essa arvore de classe phylo4 para poder extrair as tipLabels
    labels <- as.character(tipLabels(tree_phy4))
    on_tree <- intersect(spatial_names,labels)   # especies do raster que estao na arvore
    subtree <- keep.tip(tree, on_tree) # arvore com o subconjunto de especies
    stack_reord <- pres_bin_stack[[subtree[["tip.label"]]]] # reordenar o stack de especies de acordo com a ordem das especies na arvore
    stack_reord_t <- rast(stack_reord) # lendo como um spatraster para se adequar ao pacote Terra
    species_name <- names(stack_reord)
    subtree <- phylo4(subtree)
    branch_length <- as.numeric(edgeLength(subtree, 1:nTips(subtree)))
  }
