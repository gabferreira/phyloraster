...
vignette: "%\\VignetteIndexEntry{phylogrid-vignette} %\\VignetteEngine{knitr::rmarkdown}
...
pdr <- phylogrid::rast.pd(x = dataprep$x, branch.length = dataprep$branch.length, filename = NULL)
pdr[[1]]
```
...
per <- phylogrid::rast.pe(x = dataprep$x, branch.length = dataprep$branch.length, filename = NULL)
per
```
