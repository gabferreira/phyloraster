...
The intern function `spat.rand` randomizes raster stacks through three different methods (site, species or fullspat) that can be chosen through the argument random. 
The first method (site) keeps species richness constant in each pixel, but randomizes the position of species in the stack. To perform this randomization, we based on the frequency of presences and absences for each species. Thus, the size of the distribution will not change, but the position of the species in space. In the second method (species), the order of species in the stack is kept constant, but the pixels where each species are present are randomized in space. This method is based on sampling of the presences and absences in each pixel. The third (fullspat) combines randomization by site and species at the same time. In turn, for phylogenetic randomization, we provide the function tip.rand that shuffle taxa names across all taxa included in phylogeny. 
