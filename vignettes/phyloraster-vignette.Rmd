...
`phylorgid` brings an alternative to these packages by providing functions that calculate diversity and endemism metrics for each raster cell, reducing the amount of RAM required for data processing.
...
The development version of `phyloraster` can be downloaded from [GitHub](https://github.com/gabferreira/phyloraster).
...
dataprep <- phyloraster::phylo.pres(x = ras, tree = tree)
```
...
Great!!
Now, we are already able to calculate the measures of endemism and evolutionary diversity.
...
```{r, warning= FALSE}
ras <- terra::rast(system.file("extdata", "rast.presab.tif", package = "phyloraster"))
wer <- phyloraster::rast.we(x = ras, filename = NULL)
wer
```
...
pdr <- phyloraster::rast.pd(x = dataprep$x, branch.length = dataprep$branch.length, filename = NULL, cores = 1)
pdr[[1]]
```
...
per <- phyloraster::rast.pe(x = dataprep$x, branch.length = dataprep$branch.length, filename = NULL, cores = 1)
per
```
...
The function bootspat_str() from the package SESraster keeps the richness exact and samples species presences proportionally to their observed frequency (i.e. number of occupied pixels).
The randomization will not assign values to cells with nodata.
...
t <- rast.pd.ses(data$x, data$branch.length, aleats = 10, random = "spat_str")
```
...
The package also brings the function `delta.grid` that allows you to calculate the difference between spatialized diversity metrics between two different times.
...
and Monro, A.
(2001).
Endemism in theAustralian flora.
